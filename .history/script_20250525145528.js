...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const calMinimizeBtn = document.getElementById('calendar-minimize-btn');

  if (!calendarApp || !calMinimizeBtn) return;

  calMinimizeBtn.addEventListener('click', () => {
    calendarApp.classList.toggle('minimized');

    // update the icon/title if you like
    if (calendarApp.classList.contains('minimized')) {
      calMinimizeBtn.textContent   = '▢';
      calMinimizeBtn.title         = 'Maximize calendar';
      calMinimizeBtn.setAttribute('aria-label', 'Maximize calendar');
    } else {
      calMinimizeBtn.textContent   = '_';
      calMinimizeBtn.title         = 'Minimize calendar';
      calMinimizeBtn.setAttribute('aria-label', 'Minimize calendar');
    }
  });
});
