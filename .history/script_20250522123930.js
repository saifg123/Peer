...
calendarEl.addEventListener("drop", e => {
  ...
  requestAnimationFrame(() => {
    ...
    if (note) {
      // Remove any lingering animationend listeners by cloning
      const newNote = note.cloneNode(true);
      note.parentNode.replaceChild(newNote, note);

      newNote.classList.remove("drop-bounce"); // Clear any old state first
      void newNote.offsetWidth; // Force reflow to restart animation
      newNote.classList.add("drop-bounce");

      // Ensure the class gets removed when the animation ends
      newNote.addEventListener("animationend", () => {
        newNote.classList.remove("drop-bounce");
      }, { once: true });
    }
  });
});
