...
html += `<div class="month-events">`;
        ...
        if (notes[dateStr]) {
          html += `<div class="note-banner" draggable="true" data-note-key="${dateStr}">${notes[dateStr]}</div>`;
            ...
            // Show note banners for drag-and-drop (same as day view: .note-banner, draggable="true" for drag-and-drop animation)
            // If there's a note for the day, show it; otherwise, show the first hour note found
            if (notes[dateStr]) {
            html += `<div class="note-banner" draggable="true" data-note-key="${dateStr}">${notes[dateStr]}</div>`;
            } else {
            for (let h = 0; h < 24; h++) {
...
/* Animate note-banner when dragging */
calendarEl.addEventListener("dragstart", (e) => {
  const banner = e.target.closest(".note-banner");
  if (banner) {
...
calendarEl.querySelectorAll(".note-banner").forEach(el => {
  el.addEventListener("dragstart", e => {
    e.dataTransfer.setData("text/plain", el.dataset.noteKey);
  });
});
