...
let draggedNoteKey = null;    // ← ADD THIS
...
calendarEl.addEventListener("wheel", e=>{
  ...
  scrollDebounce = setTimeout(()=>scrollDebounce=null,400);
  zoom(e.deltaY>0?1:-1);
},{passive:false});
  ...
  for (let row = 0; row < 6; row++) {
    for (let col = 0; col < 7; col++) {
      if ((row === 0 && col < firstDay) || d > total) {
        ...
        initializeDay(dateStr);
        // all hours
        // (No-op, kept for compatibility. Notes are rendered below with .note-banner and draggable="true" for drag-and-drop animation)
...
html += `<div class="month-events">`;
        ...
        // Show note banners for drag-and-drop (same as day view: .note-banner, draggable="true")
    // Only show one note: either full-day note or first hour note
let shownNote = false;
if (notes[dateStr]) {
  html += `<div class="note-banner" draggable="true" data-note-key="${dateStr}">${notes[dateStr]}</div>`;
  shownNote = true;
...
animate();
// ─── Drag-and-Drop Notes (Single Handler Set) ───
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggleBtn    = document.getElementById('chatbox-minimize-btn');
  const headerStrip  = document.getElementById('chatbox-header');

  if (!chatbox || !toggleBtn || !headerStrip) return;

  // Update the toggle button’s icon/title based on state
  function refreshToggleUI() {
    if (chatbox.classList.contains('minimized')) {
      toggleBtn.textContent = '▢';
      toggleBtn.title       = 'Maximize chat';
      toggleBtn.setAttribute('aria-label', 'Maximize chat');
    } else {
      toggleBtn.textContent = '_';
      toggleBtn.title       = 'Minimize chat';
      toggleBtn.setAttribute('aria-label', 'Minimize chat');
    }
  }

  // Click on the button
  toggleBtn.addEventListener('click', (e) => {
    // You *could* stopPropagation here if you have other nested clicks,
    // but it isn’t strictly necessary unless you run into weird side-effects.
    chatbox.classList.toggle('minimized');
    refreshToggleUI();
  });

  // Click on the header (single-click)
  headerStrip.addEventListener('click', (e) => {
    // Don’t re-toggle if the button itself was clicked
