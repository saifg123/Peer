...
function renderDay(){
  ...
  for(let h=0; h<24; h++){
    ...
    html += `
      <div class="day-cell" data-hour="${h}">
        <span>${h}:00</span>
        ${ notes[key]
           ? `<div class="note-banner">${notes[key]}</div>`
           : `` }
      </div>
    `;
  }
...
function renderWeek(){
  ...
  for(let d=0; d<7; d++){
    ...
    const key = formatKey(new Date(dt.getFullYear(),dt.getMonth(),dt.getDate(),0,0,0));
    html += `
...
function renderMonthGrid(){
  ...
  // find the Sunday before (or of) the 1st
  ...
  let html = `<div class="month-grid"><div class="month-labels">`
           + ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"]
             .map(d=>`<div class="month-label">${d}</div>`).join("")
           + `</div>`;
  ...
  for(let w=0; w<6; w++){
    ...
    for(let i=0; i<7; i++, d.setDate(d.getDate()+1)){
      const inMonth = d.getMonth()===month,
            key     = formatKey(new Date(d.getFullYear(),d.getMonth(),d.getDate(),0,0,0));
      html += `
...
// ─── keep your existing dblclick handler intact ───
// calendarEl.addEventListener("dblclick", …)
// After wrapper.innerHTML = renderView(...)
const btn = wrapper.querySelector("#open-note-day");
if (btn) {
  btn.onclick = () => {
    // Only in Day view (should always be true here)
    selectedDate = new Date(currentDate);
    const key = selectedDate.toISOString().slice(0,10);
    modalDate.textContent = selectedDate.toDateString();
    modalText.value        = notes[key] || "";
