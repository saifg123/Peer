...
function onCellClick(e) {
  ...
  if (view === "month") {
    ...
    // FULL-DAY NOTE (click on day number or note)
    const dayNumEl = mcell.querySelector(".month-day-number");
    if (dayNumEl && (e.target === dayNumEl || e.target.closest(".note-banner"))) {
    ...
    // DRILL → WEEK (click on cell background)
    const dayNum = parseInt(mcell.querySelector(".month-day-number")?.textContent, 10);
  ...
  if (view === "week") {
    ...
    const rowIdx = Array.from(row.parentElement.children).indexOf(row) - 1; // -1 for .week-labels
    if (rowIdx < 0) return; // skip label row

    const start = new Date(currentDate);
    start.setDate(start.getDate() - start.getDay() + colIdx);
