...
calendarEl.addEventListener("dragstart", (e) => {
  ...
  const noteKey = banner.dataset.noteKey;
  if (!noteKey) return;
...
calendarEl.addEventListener("dragover", (e) => {
  if (e.target.closest(".month-cell") || e.target.closest(".week-cell")) {
...
calendarEl.addEventListener("drop", (e) => {
  ...
  const toKey = target.dataset.date;
...
calendarEl.addEventListener("dragover", (e) => {
  if (e.target.closest(".day-cell") || e.target.closest(".week-cell")) {
    e.preventDefault(); // allow drop
  }
});

calendarEl.addEventListener("drop", (e) => {
  const cell = e.target.closest(".day-cell, .week-cell");
  if (!cell) return;

  const newKey = cell.dataset.date;
  const oldKey = e.dataTransfer.getData("text/plain");

  if (newKey && oldKey && newKey !== oldKey && notes[oldKey]) {
...
calendarEl.addEventListener("dragstart", (e) => {
  const banner = e.target.closest(".note-banner");
  if (!banner) return;
  const key = banner.dataset.noteKey;
...
calendarEl.addEventListener("drop", (e) => {
  ...
  animate(); // Re-render view to reflect change
});
