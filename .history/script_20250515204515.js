...
function shiftQuarter(dir){
  ...
  if (q<0) q=0;
  let newQ=(q+dir+4)%4, newYear=y+Math.floor((starts[q]+dir*3)/12);
  currentDate=new Date(newYear, starts[newQ],1);
}
function shiftSemester(dir){
  const m=currentDate.getMonth(), y=currentDate.getFullYear();
...
function onCellClick(e){
  ...
  // DAY: double-click for event modal
}
...
// Double-click on grid to open modal
...
function labelFor(view){ /* ... your existing */ }
