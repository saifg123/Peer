...
function renderDay() {
  ...
  // ✅ Render note ONCE for the whole day (just in hour 0, or anywhere you want)
  if (h === 0 && notes[dateStr]) {
    html += `<div class="note-banner">${notes[dateStr]}</div>`;
  }
...
document.getElementById('save-note').addEventListener('click', () => {
  const noteText = document.getElementById('event-modal-text').value.trim();
  if (!noteText) return;

  const targetCell = window.currentTargetCell;
  if (targetCell) {
    const noteEl = document.createElement('div');
    noteEl.className = 'note-banner';
...
// Triggered from a button in the header or cell
function openNoteModalForCell(cellElement) {
  window.currentTargetCell = cellElement;

  const modal = document.getElementById('event-modal');
  modal.classList.remove('hidden');
  document.getElementById('event-modal-text').focus();
}
document.querySelectorAll('.day-cell, .week-cell').forEach(cell => {
  cell.addEventListener('dblclick', () => {
    openNoteModalForCell(cell);
  });
});
function makeNoteDraggable(noteEl, noteText) {
  noteEl.draggable = true;
  noteEl.addEventListener('dragstart', (e) => {
    e.dataTransfer.setData('text/plain', noteText);
    e.dataTransfer.effectAllowed = 'move';
...
makeNoteDraggable(noteEl, noteText);
