...
const notes = {}; // now keys will be like "2025-05-17T14"
...
function animate(){
  ...
  modal.dataset.noteKey = key; // <-- THIS LINE WAS MISSING
...
function renderDay() {
  ...
  // ✅ Show note banner ONCE
  if (notes[dateStr]) {
    html += `<div class="note-banner" draggable="true">${notes[dateStr]}</div>`;
 ...
 for (let h = 0; h < 24; h++) {
  ...
  html += `<div class="day-cell" data-date="${hourKey}"><span>${h}:00</span>`;
  if (notes[hourKey]) {
    html += `<div class="note-banner" draggable="true" data-note-key="${hourKey}">${notes[hourKey]}</div>`;
  }
  html += `</div>`;
}
...
function renderWeek() {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const hourStr = String(h).padStart(2, '0');
const hourKey = `${dateStr}T${hourStr}`;
      html += `<div class="week-cell" data-date="${hourKey}">`;
  ...
  for (let row = 0; row < 6; row++) {
    for (let col = 0; col < 7; col++) {
      if ((row === 0 && col < firstDay) || d > total) {
        ...
        html += `<div class="month-cell" data-date="${dateStr}"><div>${d}</div>`;
html += `<div class="month-events">`;
     ...
     if (notes[dateStr]) {
  html += `<div class="note-banner" draggable="true" data-note-key="${dateStr}">${notes[dateStr]}</div>`;
} else {
  for (let h = 0; h < 24; h++) {
...
modalSave.onclick = () => {
  ...
  // Force re-render to reflect saved note
...
calendarEl.addEventListener("dblclick", e => {
  ...
  const hourKey = cell.dataset.date;
  if (!hourKey) return;

  selectedDate = new Date(hourKey);
  modalDate.textContent = selectedDate.toString();
  modalText.value = notes[hourKey] || "";

  modal.dataset.noteKey = hourKey;
  modal.classList.remove("hidden");
});
...
calendarEl.addEventListener("dragstart", (e) => {
  ...
  if (!banner) return;
  e.dataTransfer.setData("text/plain", banner.dataset.noteKey || "");
...
calendarEl.addEventListener("drop", (e) => {
  ...
  const value = notes[oldKey];
  if (!value) return;

  // Move note
  notes[newKey] = value;
  delete notes[oldKey];
  animate(); // re-render view
});
...
document.addEventListener("mousedown", (e) => {
  const note = e.target.closest(".note-banner");
  if (note) {
    e.preventDefault();
    ...
    draggingNote = note.cloneNode(true);
    draggingNote.classList.add("smooth-dragging");
    draggingNote.style.left = rect.left + "px";
    draggingNote.style.top = rect.top + "px";
    document.body.appendChild(draggingNote);
...
document.addEventListener("mouseup", (e) => {
  if (draggingNote) {
    // Detect cell under mouse
    const elementUnder = document.elementFromPoint(e.clientX, e.clientY);
    // Accept drop on cells with data-date attribute (day-cell, week-cell, etc.)
    const cell = elementUnder?.closest("[data-date]");

    if (cell && originalNote) {
      const fromKey = originalNote.getAttribute("data-note-key") || cell.getAttribute("data-date");
      const toKey = cell.getAttribute("data-date");
      if (fromKey && toKey && fromKey !== toKey) {
        notes[toKey] = notes[fromKey];
        delete notes[fromKey];
        animate(); // re-render to reflect the move
      }
    }
...
function animateDrag() {
  ...
  const currentX = parseFloat(draggingNote.style.left || 0);
  const currentY = parseFloat(draggingNote.style.top || 0);
  const newX = currentX + (mouseX - offsetX - currentX) * 0.2;
  const newY = currentY + (mouseY - offsetY - currentY) * 0.2;
...
// Enable drop by preventing default
calendarEl.addEventListener("dragover", (e) => {
  e.preventDefault(); // necessary to allow dropping
});

// Handle drop: move the note to new location
calendarEl.addEventListener("drop", (e) => {
  e.preventDefault();
  const oldKey = e.dataTransfer.getData("text/plain");
  ...
  const noteContent = notes[oldKey];
  if (!noteContent) return;

  delete notes[oldKey];
  notes[newKey] = noteContent;
  animate(); // re-render to show note in new location
});
...
// Allow drop
calendarEl.addEventListener("dragover", (e) => {
  if (!e.target.closest(".day-cell, .week-cell")) return;
  e.preventDefault();
});

// Handle drop
calendarEl.addEventListener("drop", (e) => {
  const dropTarget = e.target.closest(".day-cell, .week-cell");
  if (!dropTarget || !draggedNoteKey) return;
  e.preventDefault();

  const newKey = dropTarget.dataset.date;
  const text = notes[draggedNoteKey];
  if (!text || !newKey || newKey === draggedNoteKey) return;

  // Move note to new key
  delete notes[draggedNoteKey];
  notes[newKey] = text;

  draggedNoteKey = null;

  animate(); // rerender view
});
