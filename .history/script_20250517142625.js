...
function renderWeek() {
  // header row of weekday labels + full-day banners
  const start = new Date(currentDate);
  start.setDate(start.getDate() - start.getDay());
  let weekLabels = `<div class="week-labels">`;
  for (let d = 0; d < 7; d++) {
    const dt = new Date(start);
    dt.setDate(start.getDate() + d);
    const dayKey = formatKey(new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), 0,0,0));
    weekLabels += `
      <div class="week-label">
        ${dt.toLocaleDateString(undefined,{weekday:"short",day:"numeric"})}
        ${notes[dayKey]
          ? `<div class="note-banner">${notes[dayKey]}</div>`
          : ``
        }
      </div>
    `;
  ...
  return `<div class="week-grid">${weekLabels}${rows}</div>`;
}
...
function renderMonthGrid() {
  ...
  ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].forEach(d=>{
    html += `<div class="month-label">${d}</div>`;
  });
